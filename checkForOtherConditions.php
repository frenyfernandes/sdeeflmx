<?php
    ...
    if (!function_exists('curl_init')){
		...
		foreach ($matchIds as $key => $matchId) {
            ...
            if($match->attributes()->status == "FT"){
						...
						} /*else if($match->attributes()->status == "Aban." || $match->attributes()->status == "Cancl." || $match->attributes()->status == "Susp."){
								echo $match->attributes()->id . ": " . "delete this match and notify". "<br/>";
						} else if($match->attributes()->status == "Postp."){
                echo $match->attributes()->id . ": " . "this match is postponed". "<br/>";
						} else if($match->attributes()->status == "AET"){
               //echo "matches are not postponed abandoned neither over";
						}*/ else {
	...
	function curl_download_cancel($Url, $matchIds){
		$cancel = array();
    ...
    if (!function_exists('curl_init')){
		...
		foreach ($matchIds as $key => $matchId) {
			foreach ($xml->league as $league) {
				foreach ($league->match as $match) {
					if($match->attributes()->id == $matchId){
            if($match->attributes()->status == "Aban." || $match->attributes()->status == "Cancl." || $match->attributes()->status == "Susp." || $match->attributes()->status == "Postp." || $match->attributes()->status == "Delayed"){
            	   $cancel[(string)$match->attributes()->id] = (string)$match->attributes()->id;
								//echo $match->attributes()->id . ": " . "delete this match and notify". "<br/>";
						}/* else if($match->attributes()->status == "Postp."){
							  $cancel['suspended'] = $match->attributes()->id;
                echo $match->attributes()->id . ": " . "this match is postponed". "<br/>";
						} else if($match->attributes()->status == "AET"){
               //echo "matches are not postponed abandoned neither over";
						} */else {
  ...
  $CancellationList = curl_download_cancel("http://www.tipgin.net/datav2/accounts/mbulut/soccer/livescore/livescore.xml", $matchIds);
  
  /* code to get bet ids for canceled matches*/
	foreach ($CancellationList as $value) {
    $queryCancelBets = "SELECT bet_id FROM bets WHERE match_id = $value";
	  $resultCancelBets = mysqli_query($connection, $queryCancelBets);
	  while ($row1 = mysqli_fetch_assoc($resultCancelBets)) {
		$cancelBetIds[] = $row1["bet_id"];
		echo "bet id's corresponding to canceled match ".$row1["bet_id"];
		$Url = "http://betterthegame1-1.elasticbeanstalk.com/REST/webService/deleteBets;betId=".$row1["bet_id"];
  	cancelBets($Url);
		print_r($cancelBetIds);
		
	  }
	}
  
  /* loop isnt  to set url seperatel is not needed*/
  /*foreach($cancelBetIds as $value){
  	//echo $value;
  	$Url = "http://betterthegame1-1.elasticbeanstalk.com/REST/webService/deleteBets;betId=".$value;
  	//cancelBets($Url);
  	//cancelBets(http://betterthegame1-1.elasticbeanstalk.com/REST/webService/deleteBets;betId=855
  }*/
 

	echo "\nWinners List is as follows: \n";
  print_r($winnerList);echo "<br>";

	echo "\nThe cancellation list is: \n";
  print_r($CancellationList);echo "<br>";
